...
*************************************************************************/
trigger Expense_AdvanceWorkflow on pse__Expense__c (after insert,after update){
    ...
    if (AW_AdvanceWorkflowExecutionEngine.EvaluationIsRunning==false){
        if(Trigger.isUpdate)
           ...
           AW_AdvanceWorkflowExecutionEngine.startRulesProcessing(Trigger.Newmap.keyset(),'pse__Expense__c',Trigger.isInsert);    
    }    
}