...
*************************************************************************/
trigger RateCard_AdvanceWorkflow on pse__Rate_Card__c (after insert,after update){
    ...
    if (AW_AdvanceWorkflowExecutionEngine.EvaluationIsRunning==false){
        if(Trigger.isUpdate)
           ...
           AW_AdvanceWorkflowExecutionEngine.startRulesProcessing(Trigger.Newmap.keyset(),'pse__Rate_Card__c',Trigger.isInsert);    
    }    
}